...
#class ConstrainedTarget(models.Model):
...
class Constraint(models.Model):
    ...
    target = models.ManyToManyField('Target')# , through=ConstrainedTarget)
    annotation = models.ForeignKey('Annotation', related_name='constraints')

#class AnnotatedTarget(models.Model):
...
class Annotation(models.Model):
    ...
    modification_date = models.DateTimeField(auto_now=True, )#default=datetime.datetime.now())

    type = models.CharField(max_length=10, choices=TYPE_CHOICES, default='Comment')

    # should be named author
    author = models.ForeignKey(Profile) # User

    target = models.ManyToManyField(Target)#, through=AnnotatedTarget)
    ...
    has_answers = models.BooleanField(default=False)#, editable=False)
    ...
    def save(self):
        #self.get_book(save_if_not_exists=True)
